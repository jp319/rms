...
const router = createRouter()
  .openapi(contracts.list, async (c) => {
    ...
    const data = await propertiesService.list(owner.id);
  ...
  .openapi(contracts.getOne, async (c) => {
    ...
    const data = await propertiesService.get(id, owner.id);
    return c.json({ data }, StatusCodes.OK);
  })
  ...
  .openapi(contracts.listUnits, async (c) => {
    const owner = checkOwner(c);
    const { id } = c.req.valid("param");
    const data = await propertiesService.listUnits(id, owner.id);
    return c.json({ data }, StatusCodes.OK);
  })
  ...
  .openapi(contracts.listPropertyImages, async (c) => {
    const owner = checkOwner(c);
    const { id } = c.req.valid("param");
    const data = await propertyImagesService.list(id, owner.id);
    return c.json({ data }, StatusCodes.OK);
  });
