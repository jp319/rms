...
export const propertyImagesService = {
  ...
  // 3. List
  list: async (propertyId: number, ownerId: number) => {
    const property = await propertiesRepository.findByIdAndOwnerId(
      propertyId,
      ownerId,
    );
    ...
    return await propertyImagesRepository.findManyByPropertyId(property.id);
  },

  // 4. Delete
  delete: async (imageId: number, ownerId: number) => {
    // Verify Image exists
    const image = await propertyImagesRepository.findByIdAndOwnerId(
      imageId,
      ownerId,
    );
