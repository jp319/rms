...
const router = createRouter()
  .openapi(contracts.list, async (c) => {
    ...
    const data = await unitsService.list(owner.id);
  ...
  .openapi(contracts.getOne, async (c) => {
    ...
    const data = await unitsService.get(id, owner.id);
    return c.json({ data }, StatusCodes.OK);
  })
  ...
  .openapi(contracts.update, async (c) => {
    ...
    const data = await unitsService.update(id, owner.id, updates);
    return c.json({ data }, StatusCodes.OK);
  })
  ...
  .openapi(contracts.listLeases, async (c) => {
    const { id } = c.req.valid("param");
    const owner = checkOwner(c);
    const data = await unitsService.listLeases(id, owner.id);
    return c.json({ data }, StatusCodes.OK);
  })
